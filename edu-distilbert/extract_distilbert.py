...
"""
将m（m=12或24）层的bert转化成n（n<m）层的bert 
Preprocessing script before training DistilBERT.
Specific to BERT -> DistilBERT.

conda activate py38
cd /data/xiaoyichao/projects/DistilBERT/examples/research_projects/distillation/scripts
python extract_distilbert.py --dump_checkpoint /data/xiaoyichao/projects/DistilBERT/examples/research_projects/distillation/serialization_dir/tf_bert-base-uncased_20220928.pth

python extract_distilbert.py \
--dump_checkpoint /data/search_opt_model/topk_opt/distilbert/serialization_dir/tf_bert-base-uncased_20220928.pth \
--model_name /data/search_opt_model/chinese_L-12_H-768_A-12_pretrain_pytorch
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dump_checkpoint", default="/data/xiaoyichao/projects/DistilBERT/examples/research_projects/distillation/serialization_dir/tf_bert-base-uncased_20220928.pth", type=str)
    parser.add_argument("--vocab_transform", action="store_true")
    args = parser.parse_args()

    if args.model_type == "bert":
        model = BertForMaskedLM.from_pretrained(args.model_name) # 这个位置可以换成模型的路径
        prefix = "bert"
    else:
